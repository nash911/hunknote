...
[RULES]
1. Reference ONLY hunk IDs from the inventory above (including R_* and D_* IDs from the FILE OPERATIONS section, if present)
...
8. R_* entries must be in the SAME commit as any hunks that update imports or references to use the new file path
9. D_* entries must be in the SAME commit as any hunks that remove imports or references to the deleted file
